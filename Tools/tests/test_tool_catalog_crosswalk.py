"""Contract tests for preserving 42 runtime assets while using the workbook as catalog."""
...
CROSSWALK_PATH = os.path.join(
    TOOLS_DIR, "models_core", "data", "tool_catalog_crosswalk_v5.json"
)
...
class ToolCatalogCrosswalkTests(unittest.TestCase):
    ...
    def test_all_current_runtime_assets_are_preserved_once(self):
        ...
        self.assertEqual(len(mapped), 42)
    ...
    def test_registry_exposes_dual_identity_and_dual_counts(self):
        ...
        self.assertEqual(counts["runtime_tool_count"], 42)
        self.assertEqual(counts["catalog_coverage_count"], 38)
